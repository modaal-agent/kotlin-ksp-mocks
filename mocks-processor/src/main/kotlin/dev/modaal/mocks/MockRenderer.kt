...
object MockRenderer {
  ...
  fun render(target: MockTarget): Rendering {
    ...
    for (bookkeeping in functions) {
      val block = renderFunction(bookkeeping)
      body.append(block)
      declared.claim(block, signature(bookkeeping.function))
    }
    ...
    // The rule, in the file: an agent reading a generated mock learns how its
    // members are named without leaving it. The overload long form is the one
    // name a reader cannot derive from the declaration, and it carries its own
    // comment (see `namingComment`).
    text.append(
  ...
  /**
   * A function's bookkeeping name, and — when that name is not the declared
   * name — the comment recording it. [bookkeeping] produces both from one
   * comparison, so a comment that disagrees with the member under it cannot be
   * emitted.
   */
  private data class Bookkeeping(
    val function: MockFunction,
    val name: String,
    val comment: String?,
  )

  private fun bookkeeping(function: MockFunction, name: String): Bookkeeping =
    Bookkeeping(function, name, if (name == function.name) null else namingComment(function, name))

  /**
   * The line above a function whose members are not named after it. An
   * overload's long form is the only name this renderer builds that a reader
   * cannot derive from the declaration in front of them.
   */
  private fun namingComment(function: MockFunction, name: String): String {
    val selector = function.parameters.joinToString(", ", "${function.name}(", ")") { it.name }
    return "  // `$selector` members are named $name* — overload of ${function.name}, parameter names appended\n"
  }
   ...
   */
  private fun withBookkeepingNames(functions: List<MockFunction>): List<Bookkeeping> {
    val named =
      functions
        .groupBy { it.name }
        .flatMap { (name, group) ->
          if (group.size == 1) return@flatMap listOf(bookkeeping(group.single(), name))
          val ordered =
            group.sortedWith(
              compareBy({ it.parameters.size }, { fn -> fn.parameters.joinToString { it.renderedType } }))
          ordered.mapIndexed { index, fn ->
            if (index == 0) bookkeeping(fn, name)
            else
              bookkeeping(
                fn, name + fn.parameters.joinToString("") { it.name.replaceFirstChar(Char::uppercase) })
          }
        }
    return named.sortedBy { it.name }
  }
  ...
  private fun renderFunction(bookkeeping: Bookkeeping): String {
    val b = StringBuilder()
    val function = bookkeeping.function
    val fn = bookkeeping.name
    ...
    bookkeeping.comment?.let { b.append(it) }
